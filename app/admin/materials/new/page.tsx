...
export default function AdminCreateMaterialPage() {
  ...
  // Handle PDF Upload Simulation (as requested by Supabase upload rules)
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setTimeout(() => {
        setUploading(false);
        // Map to a valid mock URL format ending in .pdf
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newMaterial: StudyMaterial = {
      ...
      url: category === 'PDF' || category === 'Formula sheet' || category === 'Practice set'
        ? (simulatedPath || '/uploads/sample_handout.pdf')
        : videoUrl || '#',
      isFree,
      subject,
      downloadCount: 0,
    ...
    localStorage.setItem('materials_db', JSON.stringify([...currentMaterials, newMaterial]));
  ...
  return (
    <Container size="xl" className="py-8 md:py-12">
      ...
      <SectionHeader
        title="Publish New Study Material"
        subtitle="Upload reference documents, outline concept notes, or map high-frequency speed lists."
      />
            ...
            {(category === 'PDF' || category === 'Formula sheet' || category === 'Practice set') && (
              <Card className="border border-surface-200">
                ...
                <p className="text-[11px] text-surface-450 mb-4 font-semibold">
                  Select a document file to upload to the Supabase Storage bucket. This ensures a real path is recorded.
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 p-4 border border-dashed border-surface-300 bg-surface-50 rounded-2xl justify-center text-center">
                  <label className="cursor-pointer flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs font-black text-surface-850">Choose PDF Document File</span>
                    <input
                      type="file"
                ...
                {simulatedPath && (
                  <div className="mt-4 p-3 bg-success-50 border border-success-200 text-success-850 rounded-xl text-xs font-bold flex items-center gap-2">
                    ...
                    <span>Uploaded: {uploadedFile?.name} ({fileMeta})</span>
                  </div>
                )}
              </Card>
            )}
