...
export default function AdminEditMockTestPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (item) {
        ...
        setTestType(item.testType || 'sectional');
        setDurationMinutes(String(item.durationMinutes));
        setTotalMarks(String(item.totalMarks));
        setDifficulty(item.difficulty);
        setLanguage(item.language || 'English');
        setIsFree(item.isFree);
        setPrice(item.price ? String(item.price) : '');
        setStatus(item.status || 'published');
        setNegativeMarking(String(item.negativeMarking || 0.25));
  ...
  if (loading) {
    return (
      <Container size="xl" className="py-16 text-center">
        <h3 className="text-xl font-extrabold text-surface-900 mb-2">Loading Test Config...</h3>
        <p className="text-xs text-surface-450 font-semibold mb-6">Retrieving mock exam details and question sheets.</p>
      </Container>
    );
  }
  ...
  return (
    <Container size="xl" className="py-8 md:py-12">
      ...
      <SectionHeader
        ...
        subtitle={`Update configuration, timer, and question sheet for: ${title}`}
      />
