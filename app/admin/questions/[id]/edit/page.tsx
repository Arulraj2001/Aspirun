...
export default function AdminEditQuestionPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (item) {
        ...
        setQuestionTextHindi(item.questionTextHindi || '');
        ...
        setExplanationHindi(item.explanationHindi || '');
        setDifficulty(item.difficulty);
        setMarks(String(item.marks));
        setNegativeMarks(String(item.negativeMarks));
  ...
  if (loading) {
    return (
      <Container size="xl" className="py-16 text-center">
        <h3 className="text-xl font-extrabold text-surface-900 mb-2">Loading Question Details...</h3>
        <p className="text-xs text-surface-450 font-semibold mb-6">Retrieving questions data form values.</p>
      </Container>
    );
  }
  ...
  return (
    <Container size="xl" className="py-8 md:py-12">
      ...
      <SectionHeader
        title="Edit Question"
        subtitle={`Update configuration and statement for Question ID: ${id}`}
      />
